# Backlog — Professional Interchange & GPU Acceleration

> Deferred from the original v0.17.0 plan. v0.17.0 was re-scoped to editing-model maturity and
> library hardening (see `docs/roadmap/v0-17-0/ROADMAP.md`). The interchange and GPU capabilities
> below remain planned but are not yet assigned to a milestone; promote them into a versioned
> `ROADMAP.md` when they are scheduled.
...
**Prerequisite**: a mature editing model (v0.17.0) and its follow-on model milestones.
